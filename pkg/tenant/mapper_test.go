...
func TestMapper_FilterTenantsInQuery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:       "http_requests_total{tenant_id=~\"alpha|alpha-dev\"}",
			wantErr:    false,
		},
		{
			...
			want:       "up{tenant_id=~\"beta\",instance=\"localhost:9090\"}",
			wantErr:    false,
		},
		{
			...
			want:       "rate(http_requests_total{tenant_id=~\"beta\"}[5m]) + rate(http_errors_total{tenant_id=~\"beta\"}[5m])",
			wantErr:    false,
		},
		{
