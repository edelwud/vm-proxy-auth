...
func (kd *KubernetesDiscovery) DiscoverBackends(ctx context.Context) ([]domain.BackendInfo, error) {
	...
	for _, service := range services.Items {
		...
		for _, svcPort := range service.Spec.Ports {
			if svcPort.Name == kd.config.HTTPPortName || svcPort.Name == "http" {
				port = svcPort.Port
				break
			}
		}
		...
		// Extract weight from annotations
		weight := 1
	...
	// Also discover via Endpoints for more detailed health info
...
func (kd *KubernetesDiscovery) enrichBackendsWithEndpoints(backends []domain.BackendInfo, endpoints []corev1.Endpoints) {
	for i := range backends {
		...
		for _, endpoint := range endpoints {
			if endpoint.Name == serviceName {
				...
				backends[i].Metadata["ready_addresses"] = fmt.Sprintf("%d", readyAddresses)
				backends[i].Metadata["total_addresses"] = fmt.Sprintf("%d", totalAddresses)
				break
			}
		}
	}
